...
1. **委派失败必须上报**:子代理启动失败、上游返回错误、档位不支持、运行异常 → 停下执行流,把失败现象 + 可选动作 + 建议汇报给用户,等用户决定后行动;禁止静默重试/静默换模型/静默放弃。
2. **审查问题必须由用户裁决**:code-reviewer 的 findings、reviewer 的偏差报告 → 把问题清单呈现给用户,由用户决定打回修复 / 接受 / 忽略;禁止自行判断后静默进入修复回环。

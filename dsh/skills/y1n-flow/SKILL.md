...
9. **用户决策红线(最高优先级,不受"连续执行"开关影响)**:子代理报上来的任何问题与决策点——委派失败(启动失败/上游错误/档位不支持)、审查问题(findings/偏差报告)、计划冲突(与 plan document 冲突/范围越界)、边界决策(取舍/假设/验收口径)——一律先汇报给用户(现象+影响+可选动作+建议)并等待决定,不得自己拍板;禁止静默重试、静默换模型、静默进入修复回环。唯一例外:用户已明确提前授权"这类事你定"的事项。
...
3. **审查问题必须由用户裁决**:见"核心原则"第 9 条,不受连续执行开关影响。
...
### 子代理反馈的问题一律上报(红线,不受连续执行开关影响)

子代理报上来的任何问题与决策点,主代理一律先汇报给用户并等待决定,**不得自己拍板**。包括:

1. **委派失败**:子代理启动失败、上游返回错误、档位不支持、运行异常。
2. **审查问题**:code-reviewer 的 findings、reviewer 的偏差报告。
3. **计划冲突**:实现与 plan document 冲突、范围越界、拆分失效。
4. **边界决策**:范围扩大/缩小、方案取舍、关键假设、验收口径变化。

处理流程:

1. **立即停下执行流**,不要把问题吞掉继续下一步(如 writer 反馈问题后,不得自行处理并继续实现)。
2. 向用户汇报:现象 + 影响 + 可选动作 + 你的建议。
3. 等用户明确决定后再行动;只有用户授权后才允许重试、换路由或进入修复回环。
4. 问题与决定同步记入 plan document(`[!]` 标记)。
